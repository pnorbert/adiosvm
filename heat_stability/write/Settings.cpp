...
static unsigned int convertToUint(std::string varName, char *arg)
{
    ...
    if (retval < 0)
    {
        throw std::invalid_argument("Negative value given for " + varName + ": " + std::string(arg));
    }
...
Settings::Settings(int argc, char *argv[], int rank, int nproc) : rank{rank}
{
    ...
    write_freq = convertToUint("write_freq", argv[8]);
    read_freq = convertToUint("read_freq", argv[9]);
